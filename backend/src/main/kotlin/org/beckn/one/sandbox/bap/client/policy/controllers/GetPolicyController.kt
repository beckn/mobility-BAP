...
class GetPolicyController @Autowired constructor(
  ...
  fun getCancellationPolicyV1(@RequestBody request: GetOrderPolicyDto): ResponseEntity<ProtocolAckResponse> {
    ...
    val context = getContext(request.context.transactionId, request.context.bppId, request.context.bppUri,ProtocolContext.Action.CANCEL)
  ...
  fun getRatingCategoriesV1(@RequestBody request: GetOrderPolicyDto): ResponseEntity<ClientOrderPolicyResponse> {
    ...
    val context = getContext(request.context.transactionId, request.context.bppId, request.context.bppUri, ProtocolContext.Action.RATING)
...
//  }
  ...
  private fun getContext(transactionId: String, bppId: String? = null, bppUri: String? = null, action: ProtocolContext.Action) =
    contextFactory.create(action = action, transactionId = transactionId, bppId = bppId, bppUri = bppUri)
}